...
public class DocumentServiceImpl implements DocumentService {
    ...
    @CachePut(value = "document", key = "#document.documentId")
//@CachePut缓存新增的或更新的数据到缓存，其中缓存名称为document，数据的key是document的documentId
    public Document save(Document document, HttpServletRequest request) throws Exception {

        if (document.getDocumentId() == null) {
            //判断是否有相同的名称
            List<Document> list = documentRepository.findByAndNameAndIsDeletedFalse(document.getName());
            if (list.size() > 0) {
                throw new BusinessException(ErrorCode.Document_Name_Exists);
            }
